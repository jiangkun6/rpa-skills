...
function showHelp() {
  printHeader("Kimi AI 任务 - Kimi AI Task");
  console.log();
  log("用法:", colors.bright);
  console.log();
  log("  /kimi-ai-task [选项]", colors.cyan);
  console.log();
  ...
  console.log();
  log("  /kimi-ai-task -k \"你好\"", colors.green);
  log("  /kimi-ai-task -k \"帮我写代码\" -r \"资深程序员\"", colors.green);
  log("  /kimi-ai-task -k \"分析这段代码\" -r \"代码专家\" -o \"markdown\"", colors.green);
  console.log();
...
async function search(options: AISearchOptions): Promise<void> {
  ...
  log("🚀 启动 Kimi AI 任务...", colors.yellow);
  log(`   任务 ID: ${TASK_ID}`, colors.cyan);
  console.log();
