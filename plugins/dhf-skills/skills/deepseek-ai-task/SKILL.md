...
自动调用 DHF Agent **任务**，在 DeepSeek AI 上进行搜索/问答。
...
```bash
# 基本调用
/deepseek-ai-task -k "你好"

# 带角色设定
/deepseek-ai-task -k "帮我写代码" -r "你是一个资深程序员"

# 完整参数
/deepseek-ai-task -k "分析这段代码" -r "代码专家" -o "markdown"

# 检查 DHF Agent 连接
/deepseek-ai-task --check
```
...
### 代码相关
```bash
/deepseek-ai-task -k "帮我写一个 Python 快速排序"
```

### 带角色设定
```bash
/deepseek-ai-task -k "解释什么是闭包" -r "JavaScript 专家"
```

### 完整参数
```bash
/deepseek-ai-task -k "写一篇关于 AI 的文章" -r "科技博主" -o "markdown"
```
...
| 平台 | 路径 |
|------|------|
| Windows | `%APPDATA%\dhf-agent\work\data\SElv2b\run\` |
| macOS | `~/Library/Application Support/dhf-agent/work/data/SElv2b/run/` |
| Linux | `~/.config/dhf-agent/work/data/SElv2b/run/` |

## 配置说明

脚本内置配置：
- `TASK_ID = "SElv2b"` - 任务 ID
