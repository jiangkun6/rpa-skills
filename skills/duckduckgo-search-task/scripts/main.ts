...
function showHelp() {
  printHeader("DuckDuckGo 搜索任务 - DuckDuckGo Search Task");
  console.log();
  log("用法:", colors.bright);
  console.log();
  log("  /duckduckgo-search-task [选项]", colors.cyan);
  console.log();
  ...
  console.log();
  log("  /duckduckgo-search-task -k \"大黄蜂科技\"", colors.green);
  log("  /duckduckgo-search-task -k \"大黄蜂科技\" -p 3", colors.green);
  console.log();
...
// 3. 调用任务 API
function callTaskAPI(input: TaskInput): Promise<{ success: boolean; run_id?: string; error?: string }> {
  return new Promise((resolve) => {
    const requestData = {
      jsonrpc: "2.0",
      id: Date.now(),
...
async function search(options: SearchOptions): Promise<void> {
  ...
  log("🚀 启动 DuckDuckGo 搜索任务...", colors.yellow);
  log(`   任务 ID: ${TASK_ID}`, colors.cyan);
  console.log();
